/* eslint-disable react/prop-types,max-len,no-shadow */
...
const BattleCard = (
  ...
  const [duration, setDuration] = useState(
    moment.duration((dateOfStart?.getTime() - (new Date()).getTime()), 'milliseconds')
  );
    ...
    >
      <div className="battle-card__top-container">
        ...
        <div className="battle-card__watch-time-container">
          {
            status === 'live' && (
              <span className="battle-card__match-link caption">
                WATCH LIVE!
              </span>
            )
          }
          {
            (status === 'upcoming' || status === 'finished') && (
              <>
                ...
                <div className="caption battle-card__tournament-date">
          ...
          [
            {
              ...
              'battle-card__logos-container_upcoming': (
                status === 'upcoming' || status === 'finished'
              ),
            },
          ]
      ...
      >
        <img
          src={disciplineIcon}
          className="battle-card__discipline-icon"
          alt="logo of command"
        />
      </div>
